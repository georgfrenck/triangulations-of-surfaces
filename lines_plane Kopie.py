...
def create_delta_1_sphere(num_equator, num_meridian):
    ...
    for j in range(num_meridian):
        ...
        delta_1.append([j*num_equator,111])
        out_temp=[(2*j),(2*j+1)%(2*num_meridian),(2*j+2)%(2*num_meridian)]
        delta_1.append([j*num_equator,112+out_temp[0]])
        delta_1.append([j*num_equator,112+out_temp[1]])
        delta_1.append([j*num_equator,112+out_temp[2]])
    for j in range(2*num_meridian):
        out_temp=(j+1)%(2*num_meridian)
        delta_1.append([112+j,112+out_temp])
...
def create_and_print(points_on_small_circle,points_on_large_circle):
    ...
    title='{}lines_plane.txt'.format(DEST,points_on_small_circle,points_on_large_circle)
    f=open(title,'w')
    str_delta_0='delta_0=[\n '
    for a in delta_0:
