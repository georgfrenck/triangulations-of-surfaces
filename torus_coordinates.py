...
# size of the circles:
n=8 # points on small circles
m=8 # points on large circles
...
def create_delta_1(num_n, num_m,num_g,p_left,p_right):
    ...
    for i in range(num_g-1):
        ...
        for j in range(3):
            delta_1.append([i*m*n + p_right[j],(i+1)*m*n + p_left[j]])
            delta_1.append([i*m*n + p_right[j],(i+1)*m*n + p_left[(j+2)%3]])
...
def print_for_grapher(num_n,num_m,num_g):
    ...
    title='{}grapher_surface_genus_{}_circle_{}_{}.txt'.format(DEST,genus,num_n,num_m)
    f=open(title,'w')
    f.write(string.replace('.',','))
    title='{}grapher_surface_genus_{}_circle_{}_{}_gluing.txt'.format(DEST,genus,num_n,num_m)
    f=open(title,'w')
    gluing_points=[]
    for k in range(num_g):
        for p in points_left:
            gluing_points.append(delta_0[p+k*m*n])
...
def create_and_print(points_on_small_circle,points_on_large_circle,genus_of_surface):
    ...
    title='{}surface_genus_{}_circle_{}_{}.txt'.format(DEST,genus,points_on_small_circle,points_on_large_circle)
    f=open(title,'w')
    str_delta_0='delta_0=[\n '
    for a in delta_0:
    ...
    print()
    print('The data for a triangulation on the surface of genus {} with {} 0-simplices, {} 1-simplices and {} 2-simplices has been created in {}'.format(genus,len(delta_0),len(delta_1),len(delta_2),f))
    print()
